...
(function () {
    ...
    app.controller('SampleListController', function ($rootScope, $scope, $http, $dialogs, INFO_EVENTS, SampleList) {
        ...
        this.deleteSampleHandler = function (sample) {
            var me = this;
            $dialogs.showConfirmationDialog('Delete the sample ' + model.name + ' from your collection?\nThis action cannot be undone.', {
                title: "Please confirm this action.",
                callback: function (result) {
                    if (result === 'ok') {
                        $http($rootScope.getHttpRequestConfig("DELETE", "sample-delete", {
                            headers: {'Content-Type': 'application/json; charset=utf-8'},
                            extra: model.sample_id
                        })).then(
                                function successCallback(response) {
                                    $dialogs.showSuccessDialog("The sample was successfully deleted.");
                                    $scope.samples = SampleList.deleteSample(model.id);
                                    $scope.tags = SampleList.updateTags().getTags();
                                    // $scope.filteredSamples = $scope.samples.length;
                                    me.retrieveSamplesData("my_samples", true);
                                },
                                function errorCallback(response) {
                                    debugger;
                                    var message = "Failed while deleting the sample.";
                                    $dialogs.showErrorDialog(message, {
                                        logMessage: message + " at SampleListController:deleteSampleHandler."
                                    });
                                    console.error(response.data);
                                    debugger
                                }
    ...
    app.controller('SampleDetailController', function ($state, $rootScope, $scope, $http, $stateParams, $timeout, $dialogs, INFO_EVENTS, SampleList, TemplateList) {
        //--------------------------------------------------------------------
        // CONTROLLER FUNCTIONS
        //--------------------------------------------------------------------
         ...
         * @param sample_id the id for the Sample to be retieved
         */
        this.retrieveSampleDetails = function (sample_id, force) {
            $scope.setLoading(true);

            $scope.model = SampleList.getBiocondition(sample_id);

            if ($scope.model === null || $scope.model.bioreplicates === undefined || force === true) {
                $http($rootScope.getHttpRequestConfig("POST", "sample-info", {
                    headers: {'Content-Type': 'application/json'},
                    data: $rootScope.getCredentialsParams({biocondition_id: sample_id, recursive: true})
                })).then(
                        function successCallback(response) {
                        ...
                        function errorCallback(response) {
                            ...
                            $dialogs.showErrorDialog(message, {
                                logMessage: message + " at SampleDetailController:retrieveSampleDetails."
                            });
                            console.error(response.data);
                            $scope.setLoading(false);
                        }
        ...
        this.send_create_sample = function (callback_caller, callback_function) {
            ...
            $http($rootScope.getHttpRequestConfig("POST", "sample-create", {
                headers: {'Content-Type': 'application/json'},
                data: $rootScope.getCredentialsParams({'sample_json_data': $scope.model}),
            })).then(
                    function successCallback(response) {
                        console.info((new Date()).toLocaleString() + "Sample " + $scope.model.sample_id + " successfully saved in server");
                        $scope.model.sample_id = response.data.newID;

                        SampleList.addSample($scope.model);
...
//                        $rootScope.$broadcast(INFO_EVENTS.sampleCreated);
                    ...
                    function errorCallback(response) {
                        ...
                        $dialogs.showErrorDialog(message, {
                            logMessage: message + " at SampleDetailController:send_create_sample."
                        });
                        console.error(response.data);
        ...
        this.send_update_sample = function (callback_caller, callback_function) {
            ...
            $http($rootScope.getHttpRequestConfig("POST", "sample-update", {
                ...
                data: $rootScope.getCredentialsParams({'sample_json_data': $scope.model}),
                    ...
                    function successCallback(response) {
                        console.info((new Date()).toLocaleString() + "Sample " + $scope.model.sample_id + " successfully updated in server");
                        $scope.setLoading(false);
                        callback_caller[callback_function](true);
                    },
                    function errorCallback(response) {
                        ...
                        $dialogs.showErrorDialog(message, {
                            logMessage: message + " at SampleDetailController:send_update_sample."
                        });
                        console.error(response.data);
        ...
        this.send_lock_sample = function (newViewMode) {
            ...
            $http($rootScope.getHttpRequestConfig("POST", "sample-lock", {
                headers: {'Content-Type': 'application/json'},
                data: $rootScope.getCredentialsParams({'sample_id': $scope.model.sample_id}),
            })).then(
                    function successCallback(response) {
                        if (response.data.success) {
                            ...
                            $dialogs.showErrorDialog('Apparently user ' + response.data.user_id + ' opened this object for editing. </br>Please, try again later. If the problem persists, please contact with tecnical support.', {
                                logMessage: ((new Date()).toLocaleString() + "EDITION DENIED FOR Sample " + $scope.model.sample_id)
                            });
                            $scope.setLoading(false);
                        }
                    },
                    function errorCallback(response) {
                        ...
                        $dialogs.showErrorDialog(message, {
                            logMessage: message + " at SampleDetailController:send_lock_sample."
                        });
                        console.error(response.data);
                        $scope.setLoading(false);
                    }
            ...
            $http($rootScope.getHttpRequestConfig("POST", "sample-unlock", {
                ...
                data: $rootScope.getCredentialsParams({'sample_id': $scope.model.sample_id}),
                    ...
                    function errorCallback(response) {
                        ...
                        $dialogs.showErrorDialog(message, {
                            logMessage: message + " at SampleDetailController:send_unlock_sample."
                        });
                        console.error(response.data);
                        callback_caller[callback_function](false);
                        $scope.setLoading(false);
        ...
        this.execute_tasks = function (status) {
                ...
                try {
                    switch (current_task.command)
                    {
                        ...
                        case "clear_locked_status":
                            //TODO:
                            console.info(new Date().toLocaleString() + "SENDING UNLOCK Sample " + $scope.model.sample_id + " REQUEST TO SERVER");
                            this.send_unlock_sample(this, "execute_tasks");
                            console.info(new Date().toLocaleString() + "UNLOCK Sample " + $scope.model.sample_id + " REQUEST SENT TO SERVER");
                            break;
                        default:
                            status = false;
                            break;
            ...
            //IF NO MORE TASKS AND EVERYTHING GOES WELL
                ...
                $dialogs.showSuccessDialog('Sample ' + $scope.model.sample_id + ' saved successfully');
            } else {
                status = false;
                $scope.taskQueue.unshift(current_task);
                $scope.setLoading(false);
        ...
        this.editButtonHandler = function () {
            ...
            if (!SampleList.isOwner($scope.model, current_user_id) && current_user_id !== "admin") {
                console.error((new Date()).toLocaleString() + " EDITION REQUEST DENIED. Error message: User " + current_user_id + " has not Edition privileges over the Sample " + $scope.model.sample_id);
                $dialogs.showErrorDialog("Your user is not allowed to edit this sample");
                return;
            }

            //2. SEND LOCK REQUEST
            console.info((new Date()).toLocaleString() + "SENDING EDIT REQUEST FOR Sample " + $scope.model.sample_id + " TO SERVER");
        ...
        $scope.getFormTemplate($scope, 'sample-form');

        if ($stateParams.sample_id !== null) {
