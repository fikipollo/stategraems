...
Ext.define('SL.view.senchaExtensions.FilterList', {
    extend: 'Ext.container.Container',
    cls: 'FilterListTextField',
    mixins: {bindable: 'Ext.util.Bindable', field: 'Ext.form.field.Field'},
    alias: 'widget.FilterList', overflowY: "auto",
...
//            maxHeight:50,
        ...
        return [
            {border: false, padding: 5, title: me.title, tbar: me.tbar, items: me.boundList},
            {xtype: 'button', itemId: "removeFilterButton", text: '<i class="fa fa-trash"></i> Remove filter',
                scale: 'small', hidden: true, cls: "cancelButton", tooltip: 'Remove selected filter', handler: me['onRemoveBtnClick']
            }
        ];
    },
    addFilter: function (newFilterName, newFilterValue) {
        this.store.add({'value': newFilterName});
        this.filters.push(newFilterValue);
    },
    getAllInsertedFilters: function () {
        return this.filters;
    },
    onRemoveBtnClick: function () {
        this.up('FilterList').removeSelected();
    },
    removeSelected: function () {
        ...
        for (var i = 0; i < selected.length; i++) {
            var pos = me.getStore().find('value', selected[i].data.value[0]);
            this.filters.splice(pos, 1);
            me.getStore().removeAt(pos);
        }
    ...
    onSelectChange: function (selModel, selections) {
        if (!this.ignoreSelectChange) {
            this.queryById("removeFilterButton").setVisible(selections.length !== 0);
    ...
    getValue: function () {
        return this.getAllInsertedFilters();
    },
