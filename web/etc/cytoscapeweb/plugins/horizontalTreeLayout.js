;(function($$){ 'use strict';
  ...
  function HorizontalTreeLayout( options ){
    this.options = $$.util.extend({}, defaults, options);
  }
    HorizontalTreeLayout.prototype.run = function(){
        ...
        for( var i = depths.length - 1; i >=0; i-- ){
          ...
          for( var j = 0; j < depth.length; j++ ){
            var node = depth[j];
            var aux = getPosition( node, i === depths.length - 1 );
            pos[ node.id() ] = {x:aux.y, y:aux.x};
          }
        }
      ...
      $$('layout', 'horizontalTree', HorizontalTreeLayout);
